...
1. level：配置等级，包括`dev`/`production`/`test`/`default`等，其中默认等级用于未找到指定等级配置时的下位替代，优先级低
2. service id：服务ID
3. service name：服务名，仅供展示
4. config item list：配置项，是一个列表，例如**服务基础配置**、**数据库配置**，就是一个个配置项

一个配置项：

1. config item id：配置项ID，拟根据配置项名称，通过hash计算
2. config item name：配置项名称，仅供展示
3. config item level：配置项等级，取值范围同服务配置等级
4. config sub-item list：配置子项列表，例如数据库配置，**地址**、**数据库名**等，就是一个个配置子项
...
5. is supporting：是否支持，若一个服务没有**支持的默认等级的配置**，则认为配置中心不支持该服务

配置项表：（如果不同的服务需要同名配置项，例如用户服务和云文件服务都需要数据库配置，则应为不同的服务挂载不同的配置项，例如数据库-用户、数据库-云文件）

1. config item id：与level结合，唯一
    1. 建表时，编写联合唯一约束
2. config item name
3. config item level
4. config sub-item list：完整的配置子项列表
5. is supporting：是否支持，若一个配置项没有**支持的默认等级配置**，则认为配置中心不支持该配置

实现：

配置中心通过数据库维护常规配置，包括测试配置，额外提供测试配置写文件功能

配置中心启动时，加载所有数据库记录，计算出支持的服务（服务配置有默认等级，且每个配置项都有默认等级）

当一个服务来读取配置时，首先判断是否支持，支持的，返回bytes类型配置文件

修改一个服务的配置，只支持增加、删除配置项；修改一个配置项，支持新增、修改、删除配置子项
