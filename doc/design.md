...
设计：（第一版尽量简单，不带变体规则，只有基础数独规则）

1. 后端可以编辑题目，包括新增、修改和删除（软删除）
    1. 暂时信任出题人，默认所有题目都是有解的
2. 前端做题，仅在桌面端提供
    1. 暂时不记录解题历史
3. 后端仅做辅助，解题过程、校验答案均在前端完成

小游戏系统，在登录后展示入口，进入后全屏展示可游玩的小游戏，选择数独后，小游戏名展示在左侧，右侧为游戏区域

前端提供解题辅助工具：

1. 展示和编辑提示：基础工具
2. 复制和还原当前进度：为了猜测某些单元格
...
1. 单元格有3种情况：题目给出的数字（黑色加粗）、玩家填写的数字（黑色）、未确定数字（灰色提示板）
2. 单元格有2种状态（提交题解）：正确（保持原样）、错误（数字变红）
3. 非题目给出的数字，默认展示提示板、默认编辑行为操作的是提示板，想要输入确定的数字，需要单独按键
    1. 编辑提示板，暂时仅允许编辑单个单元格，且对提供的值取反，即输入1，若原来有1则取消、没有则展示
4. 复制工具复制当前进度，经过一系列操作后，可以选择保存或放弃复制出来的进度
...
1. config item id：根据name和tag生成，不可修改
2. config item name
3. config item tag：标识该配置属于哪个服务、适用于什么场景，例如**用户服务-开发**，允许为空，为空表示该配置项为默认配置
