...
func main() {
	...
	err = rc_embedded.InitAndRegister(mconfig.GetRegistrationCenterTarget(), mconst.UID_Service_Gateway, internetAddress)
	if err != nil {
		mlog.Logger().Error("register server to rc failed", zap.Error(err))
		return
	}

	startUserServer(localAddress)
}

func startUserServer(address string) {
