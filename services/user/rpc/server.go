...
func (s *userServerImpl) Login(_ context.Context, req *rpc_impl.User_LoginReq) (*rpc_impl.User_LoginRes, error) {
    user, err := db.GetUser().QueryOneInUnlocked(model.User_UserName+" = ?", req.GetUserName())
    if err != nil {
        return nil, err
    }

    if user.Password != utils.CalcSHA256(req.Password, user.Salt) {
        return nil, errors.New(merrors.Error_InvalidAccountOrPassword)
    }
...
func (s *userServerImpl) List(_ context.Context, req *rpc_impl.User_ListReq) (*rpc_impl.User_ListRes, error) {
    if len(req.OperatorId) < 1 || req.GetPage() == nil ||
        req.GetPage().GetPageSize() < 1 || req.GetPage().GetPageNum() < 1 {
...
func (s *userServerImpl) Create(_ context.Context, req *rpc_impl.User_CreateReq) (*rpc_impl.User_CreateRes, error) {
    ...
    if operator.Permission < config.GetConfig().ARankAdminPermission ||
        operator.Permission <= uint8(req.Permission) {
        return nil, errors.New(merrors.Error_PermissionDenied)
    }
...
func (s *userServerImpl) Lock(_ context.Context, req *rpc_impl.User_LockReq) (*rpc_impl.User_LockRes, error) {
    ...
    if users[1].IsLocked {
        return nil, errors.New(merrors.Error_UserLocked)
    }
    if users[0].Permission <= users[1].Permission ||
        users[0].Permission < config.GetConfig().ARankAdminPermission {
        return nil, errors.New(merrors.Error_PermissionDenied)
    }
...
func (s *userServerImpl) Unlock(_ context.Context, req *rpc_impl.User_UnlockReq) (*rpc_impl.User_UnlockRes, error) {
    ...
    if !users[1].IsLocked {
        return nil, errors.New(merrors.Error_UserUnlocked)
    }
    if users[0].Permission <= users[1].Permission ||
        users[0].Permission < config.GetConfig().ARankAdminPermission {
        return nil, errors.New(merrors.Error_PermissionDenied)
...
func (s *userServerImpl) ModifyPermission(_ context.Context, req *rpc_impl.User_ModifyPermissionReq) (*rpc_impl.User_ModifyPermissionRes, error) {
    ...
    if users[0].Permission < config.GetConfig().SRankAdminPermission ||
        ...
        req.Permission >= uint32(config.GetConfig().SRankAdminPermission) {
        return nil, errors.New(merrors.Error_PermissionDenied)
    }

    users[1].Permission = uint8(req.Permission)
...
func verifyPwdByUserID(userID string, password string) (user *model.User, err error) {
    ...
    if user.Password != utils.CalcSHA256(password, user.Salt) {
        err = errors.New(merrors.Error_InvalidAccountOrPassword)
        return
    }
