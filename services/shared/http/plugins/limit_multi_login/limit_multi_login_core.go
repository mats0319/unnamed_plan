...
func (l *LimitMultiLogin) multiLoginVerify(userID string, source string, token string, timestamp int64) error {
	...
	index := utils.GetIndex(l.sources, source)
	if index < 0 {
...
func (l *LimitMultiLogin) setLoginInfo(userID string, source string, token string, timestamp int64) error {
	...
	if v, ok := l.loginInfoMap.Load(userID); ok {
		...
		loginInfoIns.tokens = make([]string, len(l.sources))
		loginInfoIns.timestamps = make([]int64, len(l.sources))
	}
...
func (l *LimitMultiLogin) getFlags(pattern string) (*flags, int, error) {
	flagsI, ok := l.flags.Load(pattern)
	if !ok {
		mlog.Logger().Error(mconst.Error_UnknownURI + pattern)
		return nil, http.StatusNotImplemented, errors.New(mconst.Error_UnknownURI + pattern)
	}

	flagsIns, ok := flagsI.(*flags)
	if !ok {
		mlog.Logger().Error("type assert error")
		return nil, http.StatusInternalServerError, errors.New("type assert error")
	}

	return flagsIns, http.StatusOK, nil
}
