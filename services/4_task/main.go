...
func main() {
	...
	err = rc_embedded.InitAndRegister(mconfig.GetRegistrationCenterTarget(), mconst.UID_Service_Task, internetAddress)
	if err != nil {
		mlog.Logger().Error("register server to rc failed", zap.Error(err))
		return
	}
	
	startTaskServer(localAddress)
}

func startTaskServer(address string) {
