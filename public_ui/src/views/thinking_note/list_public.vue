<template>
  <div class="list-public-thinking-note">
    <el-timeline class="lptn-content">
      <el-timeline-item
        v-for="item in notes"
        :key="item['noteID']"
        placement="top"
    ...
    <div class="lptn-bottom">
      <div class="lptnb-data">
        共&nbsp;{{ total }}&nbsp;条<br />
        每次加载&nbsp;{{ pageSize }}&nbsp;条<br />
      </div>

      <div class="lptnb-more">
        <el-button v-show="(pageNum-1) * pageSize < total" type="text" plain @click="list">
          加载更多
        </el-button>

        <span v-show="(pageNum-1) * pageSize >= total" class="lptnbm-no-more">没有更多了</span>
      </div>
    </div>
  </div>
</template>
...
export default class ListThinkingNoteByWriter extends Vue {
  ...
  private list(): void {
    thinkingNoteAxios.listPublic(this.$store.state.userID, this.pageSize, this.pageNum)
      .then(response => {
